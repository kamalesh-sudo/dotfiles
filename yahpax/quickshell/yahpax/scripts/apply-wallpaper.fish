...
set -l qt_colors_generator "$script_dir/generate-qt-colors.py"
...
# Classify the actual wallpaper pixels instead of inferring brightness from
# Pywal's color0, which may not represent the image's overall luminance.
python3 "$script_dir/classify-wallpaper.py" "$palette_image" > "$cache_dir/wallpaper-lightness"; or exit $status
...
if test "$video_mode" = 1
    ...
    printf '%s\n' "$mpvpaper_pid" > "$QS_RUNTIME_DIR/mpvpaper.pid"
    sleep 0.25
